...
// капча появлялась заново. Вместо того чтобы ловить каждый возможный
// триггер отдельно — помечаем вкладку целиком как "идёт капча" по
// URL-паттерну и держим это состояние поверх всех трёх механизмов ротации (403,
...
// Момент ПЕРВОЙ пометки капчи на вкладке, отдельно от дедлайна выше
// (тот двигается вперёд при каждом новом вызове markCfChallenge) —
// нужен, чтобы посчитать, сколько реально прошло времени между
// началом challenge и отправкой pat-токена, для проверки гипотезы
// "токен просрочился через Tor".
