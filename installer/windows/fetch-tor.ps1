...
# tor.exe. См. PLAN-CROSSPLATFORM.md, Задача 2.
#
...
#
# ВАЖНО: этот скрипт НЕ был запущен на реальной Windows и не проверен
# реальным скачиванием — в сессии, где он написан, не было ни Windows,
# ни сетевого доступа к dist.torproject.org (см. PLAN-CROSSPLATFORM.md,
# Задача 12 — открытый вопрос про тестовое окружение). Синтаксис не
# прогонялся через PowerShell-интерпретатор (его не было под рукой на
# macOS-машине, где это писалось), только вычитан вручную. Перед тем
# как считать Задачу 2 закрытой — обязательно прогнать по-настоящему
# на Windows или на раннере windows-latest в CI.
#
...
try {
    ...
    Write-Warning "НАПОМИНАНИЕ: скрипт не был реально запущен на момент написания (см. комментарий вверху файла) — нужна реальная проверка перед закрытием Задачи 2."
}
finally {
