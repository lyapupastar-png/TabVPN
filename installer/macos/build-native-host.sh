...
# сам Node.js начиная с v20) вместо стороннего упаковщика.
#
# Системный `node` на macOS (Homebrew/nodejs.org) — universal (fat)
# Mach-O с обеими архитектурами (x86_64 + arm64) в одном файле. SEA
# не компилирует бинарь заново — просто внедряет blob байткода в
# копию существующего node-бинаря через postject. postject только
# редактирует Mach-O секции, поэтому архитектура хоста не имеет
# значения — можно вырезать (`lipo -thin`) любой срез из universal
# node и внедрить blob в него. Поэтому оба бинаря (arm64 и x64)
# собираются на одном Intel-хосте, отдельная Apple Silicon машина
# или матрица раннеров не нужна.
#
